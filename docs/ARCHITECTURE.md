...
- **CI (GitHub Actions):** typecheck + lint + test on every PR; portfolio build gated by Lighthouse budgets, full axe-core, and an internal link/asset check on `/` (ADR-0009) plus a case-study content gate (ADR-0010); gitleaks secret scan. Main is always releasable.
