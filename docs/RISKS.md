...
| S-04 | CSRF on mutating requests | L | M | SameSite=Lax + fail-closed Origin check (M13-06, ADR-0007 amended): absent + mismatched Origin both 403 |
...
### S-04 · CSRF on mutating requests

Browser cookies (SameSite=Lax) plus a root-guard Origin check on every mutating method (POST/PUT/PATCH/DELETE). As of M13-06 (exam finding F-2) the check is FAIL-CLOSED: both an absent and a mismatched Origin are rejected 403 FORBIDDEN_ORIGIN, closing the earlier non-browser carve-out (a cookie-bearing non-browser client omitting Origin no longer bypasses the check). The check runs before the public bypass so login is covered; GETs are ungated (GETs-never-mutate, ADR-0007). Residual: none beyond a same-origin XSS foothold, which is out of scope here (S-02 escape-only rendering). Revisit on any new mutating surface reachable by a non-browser client that cannot set Origin - none exists today (CLIs call services directly; smoke curls send the header). Pinned by `auth.origin-required.test.ts` + the mutating-inject suite sweep.

## Privacy

### P-01 · Personal career data in a public repo

Real resume, salary targets, job criteria, gap analyses, and application history are sensitive. Boundaries (ADR-0007): `docs/profile/` gitignored **before the first commit** (git history is forever — this cannot be retrofitted); fictional `docs/profile.example/` for all tests, fixtures, demos, and screenshots; local-only database; no PII in logs (review convention). The public resume page (M2-08) is deliberately curated — no phone/address. Also noted: prompts send career data to the LLM provider; use an API tier with no training on inputs and note this in the README. Deliberate publication of a sensitivity-reviewed professional subset (case studies, M2-05; about/resume pages, M2-08) crosses a few profile-derived tokens on purpose; privacy-check's publication allowlist (ADR-0011) clears only those exact operator-cleared tokens and never a sensitive class — email, phone, salary, and home address remain detected, as do all URLs **except** the single deliberately-published professional-identity URL (the LinkedIn profile) explicitly cleared for the resume page (M2-08 carve-out); every other URL, private or contact, stays detected.

**M2-10 (2026-07-23) — resume variants are a new local-only artifact class.** A tailored variant embeds real profile data (labels, details, the rendered markdown snapshot). It lives in exactly three places: the local Postgres DB (same standing as all profile rows; dumps already gitignored), the authed local wire to the web UI, and a browser-initiated download to wherever the user saves it. **The system writes zero files** — no CLI writer, no server-side file output — so nothing real-profile-derived becomes committable. All committed tests, fixtures, and the adversarial corpus use fictional data (the Alex Rivera persona) only. No gate script or allowlist changed (ADR-0012), so no new publication surface was opened.
...
**M2-07 (2026-07-22):** two more studies (Binnie and CareerForge) were published, both `personal_ai_assisted`, taking the portfolio to **seven** and meeting the portfolio-MVP exit criterion (PLAN.md:78, at least 6 honestly-labeled studies). Binnie is a Q3-scoped architecture write-up (private repo, no household data, no metrics, so its Results carry no numbers); CareerForge is the self-referential study, marked in progress, linking the public repo/ADRs/CI as living evidence. The `PUBLISHED` allowlist is **unchanged** (still the six M2-05/M2-06 tokens): CareerForge is published *from* `docs/profile/case-studies-draft.md`, so its only collisions were its own draft headings and bold lead-ins, resolved by **excluding that publication-staging draft from privacy-check's three structural extractors** while keeping email/URL/phone/salary detection over it (ADR-0011 M2-07 amendment, shipped with two planted-FAILs). Sensitive classes stay fully detected; the CareerForge $150k/$161k prior-employer figures use the same short form and do not trip the salary probe.

**M2-08 (2026-07-22):** the home/about/resume pages were published — the first non-case-study profile content in the public tree, sourced from `docs/profile/resume.md` and the job-criteria positioning statement. No phone, no home/street address (location published metro-level only); vendor names generalized and the `$150k`/`$161k` figures kept in short form, matching the published studies. Collisions were enumerated the same way (privacy-check's own extract+subtract over the committed pages) and added to the ADR-0011 allowlist as exactly four non-sensitive identity tokens (`love's travel stops & country stores`, `nintendo of america`, `automation software engineer`, `university of washington`); a colliding skill-category format label was **rephrased** on the page rather than allowlisted, keeping the set scoped to identity/tech (the original label is described, not quoted, so this note does not re-leak it). Contact is via a publish-safe email alias (not the resume's personal address) plus the LinkedIn profile URL — the latter a narrow, exact-string URL **carve-out** (ADR-0011 M2-08 amendment), every other URL still fully detected, shipped with a two-directional planted-FAIL (a different URL still leaks; removing the entry re-leaks the real one).
...
### T-04 · Paid LLM calls on a live deployment

**Standing law, 2026-08-12 (operator).** No internet-reachable deployment ever carries an `ANTHROPIC_API_KEY` (or any paid LLM credential) until Carlos grants explicit permission. **That permission is not granted, and nothing in this register or any plan pre-grants it.** Carlos's words, quoted exactly:

> "LLM budget in keyed mode is NOT an option in the near future. We will NOT allow any paid calls on the live sites. We can add guards to mitigate all risks as possible as if it was live --- but never, ever live until permission granted. Which it is not!"

The rest of this entry summarizes the binding reading rather than quoting. The enforced floor is the demo's keyless fail-closed posture from **M10-03**: in `DEMO_MODE` a present key refuses to boot, and the LLM-draft routes carry `DEMO_DISABLED` (ADR-0023). That is a floor, not a default: a deployment is not compliant merely by having no key today; it is compliant because booting with one is refused. Guards **may** be built as if the deployment were keyed (defense-in-depth), but **building a guard never authorizes adding a key**, and no guard, test, or plan constitutes the permission. M13-05 (keyed-mode LLM budget) is parked under this law and its pre-keyed-deploy trigger cannot fire without Carlos's explicit word. Residual risk: none while the floor holds; the live failure mode this closes is a keyed deployment reachable from the internet, where cost overrun (T-03) and credential exposure (S-03) become the same incident.
