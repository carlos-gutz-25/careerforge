...
// apps/portfolio/tsconfig.json extends it — esbuild's transform of the .ts
// test files then fails hard with TSCONFIG_ERROR. The vitest nuxt ENVIRONMENT
// builds into its own test build dir, so it never writes .nuxt/ — `nuxt
// prepare` here does, once, before any test file is transformed. Mirrors
// apps/web/tests/setup/prepare-nuxt.mjs.
