...
// read-only exercises with evidence counts), the one-shot review, the 404
// state, and the rendering law (M1-02) - title / focus / notes / exercise
// titles are LLM/posting-derived and escaped interpolation only. All data
// fictional.
import { mockNuxtImport, mountSuspended } from '@nuxt/test-utils/runtime';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import type { LearningPlanResponse } from '@careerforge/core';

import LearningPlanDetailPage from '../app/pages/learning-plans/[id].vue';
import { ApiError } from '../app/utils/api-error.ts';

const { getLearningPlanMock, reviewLearningPlanMock, routeState } = vi.hoisted(() => ({
...
describe('learning plan detail page', () => {
  ...
  it('renders the plan title, its cited gaps, and read-only exercises with evidence counts', async () => {
