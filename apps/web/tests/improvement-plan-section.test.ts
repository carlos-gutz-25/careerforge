...
describe('ImprovementPlanSection', () => {
  ...
  it('previews the drafting work with a skeleton while the paid call is in flight (M8-16)', async () => {
    ...
    draftImprovementPlanMock.mockReturnValue(
    ...
    expect(wrapper.find('[data-testid="plan-drafting-skeleton"]').exists()).toBe(false);
    await wrapper.find('[data-testid="plan-draft-button"]').trigger('click');
    await wrapper.vm.$nextTick();
    expect(wrapper.find('[data-testid="plan-drafting-skeleton"]').exists()).toBe(true);
