...
import { COMPOSE_ADVERSARIAL_CORPUS, COMPOSE_ATTACK_CLASSES } from './index.ts';

// The corpus source-byte law applied to the compose corpus (the drafting
// corpus.test.ts mirror): fixture SOURCE must be printable ASCII plus
// newline/tab only; any non-ASCII codepoint MUST be a visible \uXXXX escape (the
// unicode-smuggling fixture's homoglyphs are authored exactly that way).
...
describe('compose corpus source-byte law (visible \\uXXXX escapes only)', () => {
...
describe('compose corpus shape', () => {
  it('has unique kebab-case fixture ids', () => {
    const ids = COMPOSE_ADVERSARIAL_CORPUS.map((fixture) => fixture.id);
  ...
  it('covers every declared compose attack class exactly, no strays', () => {
    const present = new Set(COMPOSE_ADVERSARIAL_CORPUS.map((fixture) => fixture.class));
    for (const attackClass of COMPOSE_ATTACK_CLASSES) {
      expect(present.has(attackClass), `no fixture for class ${attackClass}`).toBe(true);
    }
    for (const attackClass of present) {
      expect(COMPOSE_ATTACK_CLASSES).toContain(attackClass);
    }
  });
