...
import { INTERVIEW_ADVERSARIAL_CORPUS, INTERVIEW_ATTACK_CLASSES } from './index.ts';

// The corpus source-byte law applied to the interview corpus (the
// learning.corpus.test.ts mirror): fixture SOURCE must be printable ASCII plus
...
describe('interview corpus source-byte law (visible \\uXXXX escapes only)', () => {
...
describe('interview corpus shape', () => {
  it('has unique kebab-case fixture ids', () => {
    const ids = INTERVIEW_ADVERSARIAL_CORPUS.map((fixture) => fixture.id);
  ...
  it('covers every declared interview attack class exactly, no strays', () => {
    const present = new Set(INTERVIEW_ADVERSARIAL_CORPUS.map((fixture) => fixture.class));
    for (const attackClass of INTERVIEW_ATTACK_CLASSES) {
      expect(present.has(attackClass), `no fixture for class ${attackClass}`).toBe(true);
    }
    for (const attackClass of present) {
      expect(INTERVIEW_ATTACK_CLASSES).toContain(attackClass);
    }
  });
  ...
  it('every fixture requirement is quoteVerified === true (an excluded requirement could never attack)', () => {
    for (const fixture of INTERVIEW_ADVERSARIAL_CORPUS) {
      for (const requirement of fixture.requirements) {
        expect(requirement.quoteVerified, fixture.id).toBe(true);
      }
    }
  });
});
