...
// CI structural guards for the learning-plan ingress (mock provider + the REAL
// learning-plan@v1): the mechanical injection invariants hold regardless of
// model behavior -- the drafting.structural.test.ts mirror. This NEVER asserts
// "the model obeyed"; that claim lives only in the learning live pass
// (learning-adversarial-smoke).

const VALID_OUTPUT = JSON.stringify({
  title: 'placeholder plan',
  items: [{ gapRef: 'g1', focus: 'placeholder focus', priority: 'low' }],
});
...
async function runFixturePayload(payload: string, script = [{ text: VALID_OUTPUT }]) {
  ...
  await runPrompt(learningPlanV1, { untrustedData: payload }, { provider, recordCall });
  return provider;
}

describe.each(LEARNING_ADVERSARIAL_CORPUS)('learning structural guards: $id', (fixture) => {
  ...
  it('sends the frozen v1 system prompt BYTE-for-BYTE, untouched by the payload', async () => {
    ...
    expect(request?.system).toBe(learningPlanV1.system);
...
describe('forged-delimiter learning fixture: the real token defeats the forgery', () => {
  const forged = LEARNING_ADVERSARIAL_CORPUS.filter(
...
describe('fresh boundary token per learning wire call', () => {
