...
import { DRAFTING_ADVERSARIAL_CORPUS, DRAFTING_ATTACK_CLASSES } from './index.ts';

// The corpus source-byte law applied to the drafting corpus (the
...
describe('drafting corpus source-byte law (visible \\uXXXX escapes only)', () => {
...
describe('drafting corpus shape', () => {
  it('has unique kebab-case fixture ids', () => {
    const ids = DRAFTING_ADVERSARIAL_CORPUS.map((fixture) => fixture.id);
  ...
  it('covers every declared drafting attack class exactly, no strays', () => {
    const present = new Set(DRAFTING_ADVERSARIAL_CORPUS.map((fixture) => fixture.class));
    for (const attackClass of DRAFTING_ATTACK_CLASSES) {
      expect(present.has(attackClass), `no fixture for class ${attackClass}`).toBe(true);
    }
    for (const attackClass of present) {
      expect(DRAFTING_ATTACK_CLASSES).toContain(attackClass);
    }
  });
  ...
  it('every forbidden marker actually occurs in its own BUILT payload (the marker is real)', () => {
    ...
    // -- this catches a typo'd marker (or an attack string parked in an
    // excluded field) at authoring time.
    for (const fixture of DRAFTING_ADVERSARIAL_CORPUS) {
