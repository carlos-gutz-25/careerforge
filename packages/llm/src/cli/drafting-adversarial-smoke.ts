// Manual drafting adversarial live pass for `pnpm llm:drafting-adversarial-smoke`
// (ADR-0006 layer 6 at the drafting ingress, M1-12 section 5) — NEVER run by
// pnpm test; the vitest suites use the mocked provider exclusively. This is
// the standing gate for EVERY improvement-plan prompt-version bump: it runs
// the fictional drafting corpus against the REAL model with the REAL payload
...
// action, quote, canary, or payload byte.
import { DRAFTING_ADVERSARIAL_CORPUS } from '../adversarial/drafting/index.ts';
import {
...
let totalInput = 0;
let totalOutput = 0;
const verdicts: DraftingFixtureVerdict[] = [];

for (const fixture of DRAFTING_ADVERSARIAL_CORPUS) {
  ...
  try {
    ...
    verdict = evaluateDraftingFixtureRun(fixture, result, built.gapIdByRef);
  } catch {
...
// obey-marker. Any FAIL is a signal to investigate and record, never a
// silent pass.
